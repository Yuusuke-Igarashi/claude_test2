...
| | 静的デモ（GitHub Pages） | サーバ版（ローカル/常時稼働） |
|---|---|---|
| 起動 | URLを開くだけ | `npm start` |
| データ | デモ用サンプル（内蔵） | **実データ**（Mastodon公開API等） |
| SNSへの通信 | なし（自己完結の1ファイル） | あり |
...
GitHub Pages はサーバを実行できない（静的配信のみ）ため、SNSからのライブ取得はできません。
その代わり、地図・サンプル投稿・画像をすべて埋め込んだ **自己完結の `index.html`** を配信します。

- 生成: `npm run build:static` → リポジトリ直下に `index.html` を出力
- 配信: `main` への push で `.github/workflows/deploy-pages.yml` が実行され、
  生成した `index.html` だけを GitHub Pages にデプロイします
  （リポジトリ設定 → Pages → Source を **GitHub Actions** にしてください）
- 公開URL: `https://<ユーザー名>.github.io/<リポジトリ名>/`

> Pages にはサーバAPIが無いため、`index.html` は `/api/posts` を呼ばず、
> 埋め込み済みのデモデータを表示します（画面右上に `DEMOデータ` バッジ）。
