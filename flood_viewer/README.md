...
- **トラックタブ**: 入力 4（truck_traffic.ipynb の出力フォルダ）の traffic_15min.csv を、入力 1 の道路ネットワークに Id で結び付けて描く。その時刻の平時台数が 2 台以上のリンクだけを描き、有事の台数または速度が平時の 50 % 以下なら赤。ホバーでその時刻の平時・有事の台数・速度と時系列。閾値は ⚙ パネルで変更できる。平時に複数日があれば時刻ごとに平均する（network_agg.shp は使わない。集約後の代表 Id は元ネットワークの Id なので結び付く）。
- **グリッド重畳（軌跡モード）**: grid/ があれば、軌跡モードの上部の「グリッド」で指標を選ぶと、その時刻のフラグラスタ（有事 ÷ 平時が 200 % 以上 = 赤、50 % 以下 = 青、変化なし = 薄い灰、両方 0 = 透明）を背景に重ねる。時刻に連動し、カーソル位置の判定が表示される。
...
| grid/<param>_<HHMM>.tif + grid/index.json | 任意 | probe_trips.py の 100 m メッシュ増減ラスタ（+1 / −1 / 0、nodata −99）。軌跡モードで、選んだ指標（徒歩移動者数・徒歩移動距離・滞留数・引き返し数・車→徒歩数）のその時刻のラスタを道路の下に半透明で描く |
| truck/traffic_15min.csv（または入力 4 のフォルダ） | 任意 | トラックプローブのリンク統計（`probe/truck_traffic.ipynb` の出力。window, Id, Hits, AvgSp）。平時・有事の両方の日付を含むこと。日付の役割は viewer/index.json の期間、無ければ最新日 = 有事、他 = 平時 |
