...
- **トラックタブ**: 入力 3（truck_traffic.ipynb の出力フォルダ）の network_agg.shp/.dbf（集約後のリンク形状）と時刻別 traffic_YYYYMMDD_HHMM.csv を読んで描く。traj/traj_YYYYMMDD_HHMM.geojson があれば、その時刻の直近 1 時間の軌跡を平時（青）・有事（橙）で重ね、上部のチェックで切り替える（スロットごとに表示時に読む）。その時刻の平時台数が 2 台以上のリンクだけを描き、有事の台数または速度が平時の 50 % 以下なら赤。ホバーでその時刻の平時・有事の台数・速度と時系列。閾値と「または／かつ」は ⚙ パネルで変更できる。平時に複数日があれば時刻ごとに平均する。traffic_15min.csv は使わない。
...
TomTom 道路ネットワーク shp に経路として割り付け（候補リンクの列に対する最小コスト経路 = Viterbi）、
15 分ウィンドウ × リンク別の車両数 Hits と平均速度 AvgSp を出す。
...
1. ネットワーク集約: 座標列が同じ（逆向きも同じ）リンクを 1 本に統合。代表 Id = メンバー中の最小 Id。`network_agg.csv` / `.shp`。
   端点（0.1 m で丸め）からノードと隣接表を作り、リンクごとに「網の距離 250 m 以内で届くリンク」の近傍表を初回に計算してキャッシュ
2. zip を解凍せずに 1 時間分ずつ読む（`__MACOSX/` 等は除外、読めないファイルは記録して飛ばす）
3. 候補（step 5-1）: 15 分ウィンドウ × 車両の点列を 30 m / 60 秒ごとに間引き、各点から 50 m 以内で近い順に 5 本を候補にする。
   候補の無い点で点列を切る（間引き後 3 点未満の区間は使わない）
4. 経路推定（step 5-2, 5-3）: 観測コスト (点からリンクまでの距離 / 15 m)^2。遷移コストは 同一リンク = 0、接続リンク =
   1 × 乗り換え本数 + 2 × |経路距離 − 直線距離| / 直線距離、それ以外 = 対象外。点ごとに各候補の最小総コストだけを残して
   次の点へ進み（点数 × 5 × 5 の評価）、最後の点の最小コストから逆にたどった列を経路にする。どの候補にも届かない点で区間を切って再開
5. 割り付け（step 6）: 経路上の全リンクにその車両を数える（点が落ちない短いリンクも通過として数える）。速度は 1 秒毎の各点を
   経路上の最近傍リンク（50 m 以内）に割り当ててリンク × 車両で平均する（点の無い通過リンクは速度なし）。
   ウィンドウ内の最高速度 < 3 km/h の車両（駐停車）は数えない。制限速度や観測数による除外はしない
6. `traffic_YYYYMMDD_HHMM.csv`（Id, Hits, AvgSp, MedSp, n_points）、`traffic_15min.csv`、`groups.csv`（診断用、車両 ID と通過フラグ via を含む）、`summary.csv`
7. 直近 1 時間の軌跡: 15 分毎に `traj/traj_YYYYMMDD_HHMM.geojson`（HHMM = 窓の終端。5 分超の欠測で分割、5 m の Douglas-Peucker 間引き、10 m 未満は除外。車両 ID なし）。
   `AREA_GEOJSON` に区域ポリゴン（例 新宿区の tokyo.geojson、JGD2011 でも可）を与えると、点はポリゴン内、リンクは外接矩形 +1 km 内だけになり計算量が減る

```
python3 probe/test_truck_traffic.py     # 合成ネットワーク + 合成プローブ zip でノートブックのセルを順に実行し、結果を検証
```

目安（合成データでの実測: 格子状のネットワーク 19,800 リンク、500 台 × 1,000 秒 = 点 50 万、GPS 誤差 5 m）: 1 時間分の処理が約 18 秒（うち経路推定 5 秒。間引き後 22 万点）、ピークメモリ 0.6 GB。計算量は 間引き後の点数 × 候補数² に比例するので、実データで遅ければ `SAMPLE_M` を大きく（間引きを粗く）するか `AREA_GEOJSON` で範囲を絞る。
